...
<div class='link-card' style=
	...
	<?php if($_GET['page'] == 'style-guide') { ?>
		<a class='attention-voice' href="#"><h2 class="title-voice"><?=$name?></h2></a>

	<!--if it is a local project and has no case study-->
	<?php } elseif (!$hasCaseStudy && !$isExternalLink) { ?>
		<a class='attention-voice' href="?page=<?=$slug?>"><h2 class="title-voice"><?=$name?></h2></a>

	<!--if it is a local project and has a case study-->
	<?php } elseif ($hasCaseStudy && !$isExternalLink) { ?>
		<a class='attention-voice' href="?page=case-study&slug=<?=$slug?>" target=""><h2 class="title-voice"><?=$name?></h2></a>

	<!--if it's an external link'-->
	<?php } elseif ($isExternalLink) { ?>
		<a class='attention-voice' href="<?=$url?>" target="<?=$url?>"><h2 class="title-voice"><?=$name?></h2></a>
